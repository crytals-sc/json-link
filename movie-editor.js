...
// Optimized Movie Editor with corrected GitHub Commit logic
(() => {
    'use strict';
    ...
    // ĐƯỜNG DẪN TƯƠNG ĐỐI trong Repository (dùng cho GitHub API Commit)
    const DEFAULT_PATHS = ['movies.json', 'data/movies.json', 'img/lib/movies.json']; // Chỉ là path, không phải URL

    // ... (Các hàm tiện ích $, toast, encodeContent, uid, setBusy giữ nguyên)
    ...
    // DOM, State, Storage Functions (Giữ nguyên)
    const dom = {
    ...
    // ... (render, renderEpisodesEditor, readEpisodesFromEditor, form read/write, CRUD functions giữ nguyên)
    ...
    async function loadLocalFallback(){ 
        // Đã sửa lỗi: Chỉ dùng đường dẫn TƯƠNG ĐỐI nếu file nằm trong cùng dự án, hoặc 
        // sử dụng URL raw khác nếu cần. Ở đây tôi dùng một đường dẫn tương đối giả định.
        try{ const r=await fetch('../../data/movies.json'); if(!r.ok) return []; const j=await r.json(); return Array.isArray(j)?j:[]; }catch(e){return []} 
    }
    ...
    async function ghPut(token, repo, path, contentStr, branch='main', sha){
        ...
        // Nếu sha là undefined, nó sẽ không được thêm vào body, điều này là CHUẨN cho file mới.
    ...
    async function commitToGithub(){
        ...
        try{
          let file = null;
          let targetPath = 'movies.json'; // Mặc định nếu không tìm thấy

          // SỬA LỖI ĐƯỜNG DẪN: Chỉ truyền đường dẫn tương đối (path) vào ghGet
          for(const p of DEFAULT_PATHS){
            try{ 
              file = await ghGet(token, repo, p, branch); 
              if(file){ 
                targetPath=p; // Dùng path đã tìm thấy
                break; 
              } 
            }catch(e){ console.warn(`Lỗi khi tìm kiếm ${p}:`, e.message); } // Log lỗi tìm kiếm nhưng tiếp tục thử path khác
          }
          
          // Sau khi vòng lặp kết thúc, file có thể là object chứa sha (nếu tìm thấy) hoặc null (nếu không tìm thấy)
          
          const contentStr = JSON.stringify(movies, null, 2);
          dom.status && (dom.status.textContent=`Đang commit vào ${targetPath} trên branch ${branch}...`);

          // Truyền file.sha nếu file không null, ngược lại là undefined
          const res = await ghPut(token, repo, targetPath, contentStr, branch, file && file.sha ? file.sha : undefined);

          dom.status && (dom.status.textContent = `Commit thành công (${res && res.commit ? res.commit.sha.slice(0, 7) || '' : ''})`);
          toast('Commit thành công','success'); saveConf();
        }catch(err){ 
            console.error('commit err',err); 
            dom.status && (dom.status.textContent='Lỗi khi commit: '+(err.message||err)); 
            toast('Lỗi khi commit: '+(err.message||err),'error'); 
        }
        finally{ setBusy(dom.commitBtn,false); }
    }

    // ... (events và init giữ nguyên)
    function bind(){
